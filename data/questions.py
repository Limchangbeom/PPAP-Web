...
# ============================================================
# [PDF 반영 수정본]
# 아래의 "[수정-PDF]" 주석은 PPAP 진단문항 검토 최종 PDF와
# 멘토님 검토사항을 코드에 반영하면서 변경·추가·이동·삭제한 부분입니다.
# ============================================================
PAGE_SIZE = 10
...
gdpr_detailed_scope_questions = [
    ...
    multi_presence_question(
        ...
        "GDPR",
        section,
        "2-1. 이름, 이메일 주소, 계정 ID 등 그 자체로 식별력있는 정보에 해당하는가?",
        "이름, 이메일 주소, 계정 ID 등 그 자체로 식별력있는 정보에 해당하는가?",
    ...
    multi_presence_question(
        ...
        "GDPR",
        section,
        "2-2. 온라인 이용 기록, 네트워크 활동 정보 등 서비스 이용 정보에 해당하는가?",
        "온라인 이용 기록, 네트워크 활동 정보 등 서비스 이용 정보에 해당하는가?",
    ...
    multi_presence_question(
        ...
        "기존 데이터를 분석하여 자연인의 특성·성향·선호·행동 등을 추론하거나 프로파일링(자동화된 처리)을 생성하는가?",
    ...
    multi_presence_question(
        ...
        "GDPR",
        section,
        "2-7. 식별력을 완전히 상실한 정보에 해당하는가?",
        "식별력을 완전히 상실한 정보에 해당하는가?",
    ...
    single_question(
        ...
        section,
        "2-8. 마스킹 후 재식별 가능성",
        "개인정보의 일부를 삭제하는 방식으로 마스킹되었으나, 이를 복원할 수 있거나 추가 정보 결합시 재식별 가능성이 있는가?",
        ...
        visible_if=includes("gdpr_scope_detail_anonymous", "na"),
...
# [수정-PDF] GD-1-새: 익명화 여부를 pseudonym(가명) 판별과 분리하여 후속 문항으로 삽입했습니다.
add(
    single_question(
        ...
        "판단 포인트: 완전한 익명화가 확인되면 해당 데이터는 GDPR 적용 범위를 벗어납니다.",
        "GDPR 제4조 제1호, 고려사항 제26항",
        [choice("yes", "예"), choice("no", "아니오")],
...
# [수정-PDF] 최소수집 원칙은 개인정보의 범위가 아니라 실제 수집·이용의 적정성을 판단하므로 2장으로 이동했습니다.
add(
    single_question(
        "gdpr_data_minimisation", "GDPR", section,
        "최소 수집 원칙 준수 여부",
...
# [수정-PDF] GDPR 제6조 제1항의 적법근거는 열거된 6개입니다. "그 외 다른 사유"만으로는 적법근거가 되지 않도록 위반값으로 분리했습니다.
add(
    checklist_question(
        "gdpr_lawful_bases", "GDPR", section,
        "2. 다음 중 해당되는 개인정보 처리 근거",
        "다음 중 해당되는 개인정보 처리 근거를 모두 고르시오.",
        "판단 포인트: GDPR 제6조 제1항의 6개 법정 근거 중 실제 적용 근거를 하나 이상 선택해야 합니다.",
        "GDPR 제6조 제1항",
        [
            choice("consent", "정보주체의 동의"),
            choice("contract", "계약의 이행"),
            choice("legal_obligation", "법적 의무의 준수"),
            choice("vital_interests", "정보주체 또는 타인의 중대한 이익 보호"),
            choice("public_task", "공익을 위한 업무 수행 또는 공적 권한의 행사"),
            choice("legitimate_interests", "정당한 이익"),
            choice("other", "그 외 다른 사유(제6조 제1항의 적법근거 아님)"),
        ],
        required_values=[],
        one_of_groups=[["consent", "contract", "legal_obligation", "vital_interests", "public_task", "legitimate_interests"]],
        forbidden_values=["other"],
        action_hint="처리 목적마다 제6조 제1항의 법정 근거를 특정하고, '기타' 사유만으로 처리하지 마세요.",
    )
)
add(
    checklist_question(
...
add(
    checklist_question(
        ...
        [
            ...
            choice(
                "balanced",
                "정보주체의 이익·기본권 및 자유가 귀사 또는 제3자의 정당한 이익보다 우선하지 않는다(이익형량 결과)",
            ),
        ],
...
# [수정-PDF] 특별범주 개인정보는 명시적 동의 하나만 묻지 않고 GDPR 제9조 제2항의 적용 가능한 예외근거 전체를 확인합니다.
add(
    multi_presence_question(
        "gdpr_sensitive_explicit_consent", "GDPR", section,
        "3. 특별범주 개인정보 처리의 제9조 제2항 근거",
        "특별범주 개인정보를 처리하는 경우, 다음 중 적용되는 GDPR 제9조 제2항의 근거를 모두 고르시오.",
        "판단 포인트: 특별범주 개인정보는 제9조 제1항의 원칙적 금지에 대한 제2항 예외가 실제로 존재해야 합니다.",
        "GDPR 제9조 제1항·제2항",
        [
            choice("explicit_consent", "정보주체의 명시적 동의"),
            choice("employment_social", "고용·사회보장·사회보호 분야의 의무 또는 권리 수행"),
            choice("vital_interests", "정보주체 등이 동의할 수 없는 경우 중대한 이익 보호"),
            choice("nonprofit", "비영리단체의 적법한 활동 범위 내 처리"),
            choice("manifest_public", "정보주체가 명백히 공개한 개인정보"),
            choice("legal_claims", "법적 청구권의 설정·행사·방어 또는 법원의 사법작용"),
            choice("substantial_public", "중대한 공익"),
            choice("healthcare", "예방·직업의학, 진단, 보건·사회복지 등"),
            choice("public_health", "공중보건 분야의 공익"),
            choice("archive_research", "공익적 기록보존·과학/역사 연구·통계"),
            choice("none", "해당 근거 없음"),
        ],
        none_value="none", any_status="compliant", none_status="violation",
        visible_if=excludes("gdpr_scope_detail_sensitive", "no"),
        action_hint="특별범주 개인정보 처리별로 실제 적용되는 제9조 제2항 근거와 보호조치를 문서화하세요.",
    )
)

section = "3. 제3자 제공 시 별도 동의"

# [수정-PDF] NEW-GD-3-02: 외부 법인·기관·사업자에게 전송·공개·열람 허용이 있는지를 실제 분기 변수로 생성합니다.
add(single_question(
    "gdpr_third_party_exists", "GDPR", section,
    "3-0. 조직 외부 제공·접근 여부",
    "귀사는 개인정보를 조직 외부의 다른 법인·기관·사업자에게 전송·공개하거나 열람할 수 있도록 하는 행위가 있습니까?",
    "판단 포인트: 아니오이면 제3자 제공 관련 하위 문항을 건너뜁니다.",
    "GDPR 제4조 제2호·제7호·제8호",
    [choice("yes", "예"), choice("no", "아니오"), choice("unsure", "확인 필요")],
    {"yes":"recommended", "no":"recommended", "unsure":"insufficient"},
))

# [수정-PDF] 단순 기술적 경유(pass_through)는 역할 판별과 제3자 제공 하위 문항으로 내려가지 않습니다.
add(single_question(
    "gdpr_third_party_processing", "GDPR", section,
    "3-1. 처리 해당 여부",
    "현재 진행하고자 하는 행위(데이터 전송·열람 허용·공개 등)는 GDPR상 '처리'에 해당하는가?",
    "판단 포인트: 단순 기술적 경유인지 실제 개인정보 처리인지 먼저 판별합니다.",
    "GDPR 제4조 제2호",
    [choice("processing", "해당한다. 데이터 전송·열람 허용·공개 등 처리 행위이다."), choice("pass_through", "단순 기술적 경유이며 내용에 접근하지 않는다.")],
    {"processing":"compliant", "pass_through":"recommended"},
    visible_if=equals("gdpr_third_party_exists", "yes"),
))

# [수정-PDF] Processor / Independent Controller / Joint Controller의 3개 역할을 모두 구분합니다.
add(single_question(
    "gdpr_third_party_role", "GDPR", section,
    "3-2. 수령자 역할 판별",
    "정보를 넘겨받는 상대방은 귀사의 지시에 따라서만 처리하는가, 독립적으로 목적·수단을 결정하는가, 또는 귀사와 목적·수단을 함께 결정하는가?",
    "판단 포인트: 처리수탁자, 독립 컨트롤러, 공동 컨트롤러를 구분합니다.",
    "GDPR 제4조 제7호·제8호, 제26조",
    [choice("processor", "귀사의 지시에 따라서만 처리한다(Processor)"), choice("independent_controller", "수령자가 스스로 목적·수단을 결정한다(Independent Controller)"), choice("joint_controller", "귀사와 수령자가 목적·수단을 함께 결정한다(Joint Controller)")],
    {"processor":"compliant", "independent_controller":"compliant", "joint_controller":"compliant"},
    visible_if=condition_all(equals("gdpr_third_party_exists","yes"), equals("gdpr_third_party_processing","processing")),
))

# [수정-PDF] Art.28 계약 문항은 3장에 배치해 모든 Processor에 적용되도록 이동했습니다.
add(checklist_question(
    "gdpr_transfer_processor_contract", "GDPR", section,
    "3-3. 처리수탁자 계약(Art.28)",
    "수령자가 Processor인 경우 GDPR 제28조 제3항의 필수사항을 포함한 처리계약을 체결했는가?",
    "판단 포인트: 문서화된 지시에 따른 처리, 비밀유지, 처리 목적·기간·범위와 개인정보 종류, 보안, 재위탁, 권리 지원, 종료 시 삭제·반환, 감사 지원 등을 확인합니다.",
    "GDPR 제28조 제3항",
    [choice("instructions","문서화된 지시에 따라서만 처리"), choice("confidentiality","처리 권한자의 비밀유지 의무"), choice("scope","처리 목적·기간·범위와 개인정보 종류"), choice("security","제32조에 따른 보안조치"), choice("subprocessors","재위탁 통제"), choice("rights_support","정보주체 권리행사 지원"), choice("return_delete","종료 시 삭제·반환"), choice("audit","감사·입증 지원")],
    required_values=["instructions","confidentiality","scope","security","subprocessors","rights_support","return_delete","audit"],
    visible_if=equals("gdpr_third_party_role", "processor"),
))

# [수정-PDF] 공동 컨트롤러 약정은 joint_controller일 때만 표시하고 '해당 없음' 선택지를 제거했습니다.
add(single_question(
    "gdpr_joint_controller_arrangement", "GDPR", section,
    "3-4. 공동 컨트롤러 약정",
    "공동 컨트롤러에 해당하는 경우 GDPR 제26조에 따른 책임 배분 약정을 체결하고 그 요지를 정보주체에게 공개했는가?",
    "판단 포인트: 공동 컨트롤러의 책임 배분과 핵심 내용 공개를 확인합니다.",
    "GDPR 제26조",
    [choice("done", "체결 완료 및 요지 공개"), choice("missing", "미체결 또는 요지 미공개")],
    {"done":"compliant", "missing":"violation"},
    visible_if=equals("gdpr_third_party_role", "joint_controller"),
))

add(single_question(
    "gdpr_third_party_purpose_compatibility", "GDPR", section,
    "3-5. 제3자 제공 목적의 양립 가능성",
    "독립 또는 공동 컨트롤러에게 제공하는 목적이 애초 개인정보를 수집한 목적과 양립 가능하거나 별도의 적법근거를 확보했는가?",
    "판단 포인트: 원래 수집 목적과의 양립성 또는 새로운 적법근거를 확인합니다.",
    "GDPR 제6조 제4항",
    [choice("compatible","원래 목적과 동일하거나 양립 가능"), choice("new_basis","별도의 새 적법근거 확보"), choice("not_reviewed","별도 근거 미검토")],
    {"compatible":"compliant", "new_basis":"compliant", "not_reviewed":"violation"},
    visible_if=one_of("gdpr_third_party_role", ["independent_controller","joint_controller"]),
))

# [수정-PDF] Art.19 통지는 controller 역할에만 묶지 않고 실제 수령자에게 공개된 경우 적용하며 '처리제한'을 추가했습니다.
add(single_question(
    "gdpr_third_party_notification", "GDPR", section,
    "3-6. 수령자 통지 절차",
    "정보주체의 정정·삭제 또는 처리제한이 발생한 경우, 개인정보가 공개된 각 수령자에게 이를 통지하는 절차가 있으며, 불가능하거나 과도한 노력이 필요한 법정 예외는 별도로 판단하는가?",
    "판단 포인트: GDPR 제19조의 수령자 통지와 예외를 확인합니다.",
    "GDPR 제19조",
...
# [수정-PDF] '역외 재이전'이 아니라 최초 이전을 포함하는 'EU·EEA 역외 이전'으로 정리했습니다.
add(single_question(
    "gdpr_external_transfer_status", "GDPR", section,
    "3-7. EU·EEA 역외 이전 여부",
    "개인정보가 EU·EEA 밖의 제3국 또는 국제기구로 이전되거나, EU·EEA 밖에서 원격 접근되는가?",
    "판단 포인트: EU·EEA 역외 이전 여부와 제44조 이하 이전수단 확보 상태를 확인합니다.",
    "GDPR 제44조~제49조",
    [choice("eu_only","EU·EEA 역내 처리만 있음"), choice("transfer_ready","EU·EEA 역외 이전이 있고 적정성결정·SCC·BCR 등 이전수단을 확보"), choice("transfer_missing","EU·EEA 역외 이전이 있으나 이전수단 미확보"), choice("unsure","확인 필요")],
    {"eu_only":"recommended", "transfer_ready":"compliant", "transfer_missing":"violation", "unsure":"insufficient"},
    visible_if=condition_all(equals("gdpr_third_party_exists","yes"), equals("gdpr_third_party_processing","processing")),
))

add(single_question(
    "gdpr_external_transfer_special_categories", "GDPR", section,
    "3-8. 역외이전 시 특별범주 개인정보 여부",
    "EU·EEA 역외로 이전하는 데이터에 제9조 특별범주 개인정보가 포함되는가?",
    "판단 포인트: 특별범주 개인정보가 포함되면 제9조 제2항 근거도 별도로 필요합니다.",
    "GDPR 제9조, 제44조~제49조",
    [choice("no","포함되지 않음"), choice("yes_ready","포함되며 제9조 제2항 근거 확보"), choice("yes_missing","포함되나 제9조 제2항 근거 미확보")],
    {"no":"recommended", "yes_ready":"compliant", "yes_missing":"violation"},
    visible_if=condition_all(one_of("gdpr_external_transfer_status", ["transfer_ready","transfer_missing"]), excludes("gdpr_scope_detail_sensitive","no")),
))
...
add(
    single_question(
        ...
        {"yes": "compliant", "no": "violation"},
        visible_if=one_of("gdpr_external_transfer_status", ["transfer_ready", "transfer_missing"]),
    )
)
add(
    single_question(
...
# [수정-PDF] Art.28 처리수탁자 계약 문항은 3장의 gdpr_transfer_processor_contract으로 통합 관리됩니다.
add(
    checklist_question(
...
# [수정-PDF] GD-4-11: EU 이용자 대상 CI·DI 등 한국 고유 식별값의 불필요 수집 방지 설계를 확인합니다.
add(single_question(
    "gdpr_ci_eu_exclusion", "GDPR", section,
    "4-11. EU 이용자 CI·DI 불필요 수집 방지 설계",
    "EU 이용자 대상 가입·인증 절차에서 CI·DI 등 한국 고유 식별값을 불필요하게 수집하지 않도록 설계했는가?",
    "판단 포인트: CI·DI가 EU 이용자 흐름에서 불필요하게 수집되지 않도록 설계 단계에서 배제했는지 확인합니다.",
    "GDPR 제5조 제1항 (c), 제25조",
...
# [수정-PDF] GD-4-12: CI를 포함한 역외 전송 데이터에 위험 기반 보호조치를 확인합니다.
add(single_question(
    "gdpr_ci_security", "GDPR", section,
    "4-12. CI 역외전송 보호조치",
    "CI 등 고유 식별값을 포함한 데이터를 해외 마케팅·분석 도구로 전송하는 경우, 위험에 상응하는 암호화·토큰화·가명화 또는 전송 제외 등 보호조치를 적용했는가?",
    "판단 포인트: 암호화·토큰화·가명화·전송 제외·접근통제 등 실제 위험에 맞는 보호조치를 확인합니다.",
    "GDPR 제5조 제1항 (c)·(f), 제25조, 제32조",
    [choice("yes","예"), choice("no","아니오"), choice("na","해당 없음")],
    {"yes":"compliant", "no":"insufficient", "na":"recommended"},
    visible_if=one_of("gdpr_external_transfer_status", ["transfer_ready", "transfer_missing"]),
))
...
# [수정-PDF] gdpr_child_consent_quality(기존 5-6)는 중복 헤딩 성격으로 삭제했습니다.
add(
    single_question(
...
add(
    grouped_checklist_question(
        ...
        none_value="none",
        none_status="recommended",
        visible_if=condition_all(equals("gdpr_child_service", "yes"), excludes("gdpr_scope_detail_sensitive", "no")),
    )
)
add(
    checklist_question(
...
section = "6. 자동화된 결정"
# [수정-PDF] gdpr_admt_exists(6-1) 삭제: 2-5 프로파일링 응답을 재사용합니다.
add(
    single_question(
...
add(
    checklist_question(
        ...
        "판단 포인트: 사전 체크 금지, 적극적 선택, 목적별 선택, 충분한 정보를 확인합니다.",
...
# [수정-PDF] CCPA 적용 대상 사업자(Business) 요건을 캘리포니아 소비자 범위보다 먼저 확인합니다.
add(single_question(
    "ccpa_business_scope_gate", "CCPA", section,
    "0-1. CCPA Business 적용대상 확인",
    "귀사는 CCPA상 기본 사업자 요건을 충족하고, 법정 규모 기준 중 하나 이상에 해당하는가?",
    "판단 포인트: 기본 Business 요건과 현행 법정 규모기준을 함께 확인합니다.",
    "CCPA §1798.140(d); CPPA 조정 금액 기준",
    [choice("yes","예 (기본요건 및 하나 이상의 규모 기준 충족)"), choice("no","아니오 (현재 확인 범위에서는 사업자 요건 미충족)"), choice("unsure","확인 필요")],
    {"yes":"recommended", "no":"recommended", "unsure":"insufficient"},
))
# [수정-PDF] CC-0-5: 여러 장에서 반복하던 판매·공유 흐름 판별을 데이터 매핑으로 통합해 재사용합니다.
add(multi_presence_question(
    "ccpa_data_mapping", "CCPA", section,
    "0-2. 판매·공유 데이터 흐름 매핑",
    "귀사는 서비스 전반의 데이터 매핑을 분석하여, CCPA상 개인정보의 판매(Sale) 또는 공유(Share)에 해당할 수 있는 흐름을 누락 없이 판별하고 통제하고 있는가?",
    "판단 포인트: 웹/앱 트래커, 서버·오프라인 전송, 미성년자 데이터, 기타 제3자 제공 흐름을 공통 매핑합니다.",
    "CCPA §1798.140(ad), (ah), §1798.120",
    [choice("tracker","웹·앱 트래커/SDK 데이터 흐름 식별"), choice("server_offline","서버·오프라인 전송(API·배치 파일 등) 식별"), choice("minors","미성년자 데이터의 외부 마케팅·제휴 흐름 식별"), choice("other_third_party","기타 제3자 제공·대가성 데이터 흐름 식별"), choice("none","전사적 데이터 매핑 미완료")],
    none_value="none", any_status="recommended", none_status="insufficient",
    visible_if=equals("ccpa_business_scope_gate", "yes"),
))
# [수정-PDF] CC-0-2 통합: 판매·공유 옵트아웃의 링크/GPC/쿠키·타겟팅/서버·오프라인 전송 적용을 한 번에 확인합니다. 민감정보 이용제한은 별도 문항으로 유지합니다.
add(checklist_question(
    "ccpa_opt_out_system", "CCPA", section,
    "0-3. 판매·공유 통합 옵트아웃 체계",
    "식별된 개인정보의 판매·공유와 관련하여, 소비자가 채널이나 데이터 유형에 구애받지 않고 유효하게 권리를 행사할 수 있는 통합 옵트아웃 체계를 갖추고 있는가?",
    "판단 포인트: 링크 게시·작동, GPC, 쿠키/타겟팅 및 서버·오프라인 전송에 대한 전사 적용을 확인합니다.",
    "CCPA §1798.120, §1798.135",
    [choice("link_works","'Do Not Sell or Share My Personal Information' 링크 게시 및 정상 작동"), choice("gpc","GPC 등 옵트아웃 선호신호 수용"), choice("tracker_apply","전사적 적용(쿠키/타겟팅)"), choice("server_offline_apply","전사적 적용(서버·오프라인 전송)")],
    required_values=["link_works","gpc","tracker_apply","server_offline_apply"],
    visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), excludes("ccpa_data_mapping", "none")),
))
add(
    single_question(
...
add(
    checklist_question(
        ...
        visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), equals("ccpa_incentive_program", "yes")),
    )
)
add(
    single_question(
...
# [수정-PDF] CC-1-2 세부: CI 등 국내 대체식별값은 별도 문항으로 인벤토리 관리를 확인합니다. 정부 식별번호 민감정보와 자동 동일시하지 않습니다.
add(single_question(
    "ccpa_ci_inventory", "CCPA", section,
    "1-2 세부. CI 등 국내 대체식별값 인벤토리 관리",
    "CI 등 국내 대체식별값을 수집·생성·보유하는 경우 개인정보 인벤토리의 식별정보로 관리하고 있는가?",
    "판단 포인트: CI를 CCPA 정부 식별번호 민감정보로 자동 분류하지 않고 인벤토리의 식별정보로 관리하는지 확인합니다.",
    "CCPA §1798.140(v)(1)",
    [choice("yes", "예"), choice("no", "아니오"), choice("na", "해당 없음")],
    {"yes": "compliant", "no": "insufficient", "na": "recommended"},
    visible_if=equals("ccpa_business_scope_gate", "yes"),
...
for question_id, title, prompt, description, choices in [
    ...
    add(
        multi_presence_question(
            ...
            prompt,
            description,
            "CCPA §1798.140(ae)" if question_id == "ccpa_scope_sensitive" else "CCPA §1798.140(v)(1)",
            choices,
            none_value="no"
...
# [수정-PDF] 민감정보 미수집이면 자동 Skip하며, 판매·공유 옵트아웃과 별도 권리로 유지합니다.
add(single_question(
...
# [수정-PDF] Q-A에서 '잘 모르겠음' 선택 시 권장으로 끝내지 않고 미흡 판정 후 6개 예시를 재확인하게 합니다.
add(single_question(
    "ccpa_track_a_unsure_recheck", "CCPA", section,
    "NEW-CC-3-01. 판매·공유 가능성 재확인",
    "다음 사례를 다시 확인한 뒤 Q-A로 돌아가 실제 해당 여부를 선택하십시오: 광고 플랫폼 고객목록 업로드, 광고 태그 사용, 장바구니 이탈자 리타겟팅, 서버 직접 전송, 분석도구-광고계정 연동, 금전·혜택 수령 여부.",
    "판단 포인트: '잘 모르겠음'은 판매·공유 판별이 완료되지 않은 상태입니다.",
    "CCPA §1798.120, §1798.135",
    [choice("confirmed","확인했습니다 (판정: 미흡)")],
    {"confirmed":"insufficient"},
    visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), includes("ccpa_track_a_detection", "unsure")),
    action_hint="확인 후 Q-A(ccpa_track_a_detection)로 돌아가 실제 해당 항목 또는 '해당 없음'을 다시 선택하세요.",
))
add(
    single_question(
...
add(
    checklist_question(
        ...
        visible_if=condition_all(
            equals("ccpa_business_scope_gate", "yes"),
...
add(
    checklist_question(
        ...
        visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), equals("ccpa_track_b_detection", "yes")),
    )
)

section = "4. 국외이전"
# [수정-PDF] ccpa_cross_border_applies(기존 4-1) 삭제: ccpa_consumer_scope 응답을 재사용합니다.
add(
    single_question(
...
# [수정-PDF] CC-4-3: 해외 마케팅·분석 도구로 CI를 전송하는 경우 원본 형태의 불필요한 전송을 피하고 보호조치를 적용하는지 확인합니다. (기존 4-3/4-4 삭제: 데이터 매핑·옵트아웃 통합으로 대체)
add(single_question(
    "ccpa_ci_security", "CCPA", section,
    "4-3. CI 해외전송 보호조치",
    "해외 마케팅·분석 도구로 CI를 전송하는 경우, 원본 형태의 불필요한 전송을 피하고 가명화·해싱·토큰화·전송 제외 등 합리적인 보호조치를 적용하는가?",
    "판단 포인트: CI 원본 전송을 피하고 가명화·해싱·토큰화·전송 제외 등 보호조치를 적용하는지 확인합니다.",
    "CCPA §1798.140(v)(1), §1798.100(d)",
    [choice("yes", "예"), choice("no", "아니오"), choice("na", "해당 없음")],
    {"yes": "compliant", "no": "insufficient", "na": "recommended"},
    visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), excludes("ccpa_consumer_scope", "no")),
))
...
section = "5. 미성년자"
# [수정-PDF] ccpa_minors_sale_share(기존 5-0) 삭제: ccpa_data_mapping의 minors 응답을 재사용합니다.
add(
    checklist_question(
        ...
        visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), includes("ccpa_data_mapping", "minors"), excludes("ccpa_data_mapping", "none")),
    )
)
...
add(
    single_question(
        ...
        [choice("no", "아니오"), choice("yes", "예")],
        {"yes": "recommended", "no": "recommended"},
        visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), equals("ccpa_admt_exists", "yes")),
    )
)
add(
    checklist_question(
        ...
        [
            ...
            choice(
                "human", "인간 검토자가 자동결정 결과를 독립적으로 재검토하고 실제로 변경할 권한·능력을 갖는다"
            ),
        ...
        visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), equals("ccpa_admt_significant_decision", "yes")),
        # [수정-PDF] 단순 ADMT 사용이 아니라 중대한 결정에 사용되는 경우 세부통제를 표시합니다.
        action_hint="ADMT 사용사실 고지, 옵트아웃, 접근권, 영향평가를 우선 구축하고 추론정보 카테고리도 방침에 반영하세요.",
    )
)

section = "7. 쿠키"
# [수정-PDF] NEW-CC-7-01을 실제 본표에 삽입하여 ccpa_cookie_usage 변수를 생성합니다.
add(single_question(
    "ccpa_cookie_usage", "CCPA", section,
    "7-1. 쿠키·픽셀·SDK 등 추적기술 사용 여부",
    "웹사이트 또는 앱에서 쿠키, 픽셀, SDK, 광고식별자 등 소비자 활동을 추적하는 기술을 사용하는가?",
    "판단 포인트: 아니오이면 쿠키 세부통제를 건너뜁니다.",
    "CCPA §1798.140(v), §1798.135",
    [choice("yes","예"), choice("no","아니오")],
    {"yes":"recommended", "no":"recommended"},
    visible_if=equals("ccpa_business_scope_gate", "yes"),
))
add(
    checklist_question(
        ...
        visible_if=condition_all(equals("ccpa_business_scope_gate", "yes"), equals("ccpa_cookie_usage", "yes")),
    )
)
